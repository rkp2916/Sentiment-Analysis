...
Svm_classifer = SklearnClassifier(LinearSVC())
Svm_classifer.train(train_feats)
...
for tweet in neg_test:
    features = get_features_from_tweet(tweet)
    result = Svm_classifer.classify(features)
    if result == "negative":
        correct += 1
    else:
        wrong += 1
...
for item in testTweet:
    ...
    print item, Svm_classifer.classify(get_features_from_tweet(processedTestTweet))
    #print "Accuracy: {}".format(correct / float(correct + wrong))